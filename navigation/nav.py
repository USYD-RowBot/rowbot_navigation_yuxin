...
def nav():
    ...
    cruiseThrottle = config.NAV['cruise_throttle']
	...
	if Kingfisher.checkWaypointsArrived(config.NAV['dist_threshold']):
	    Kingfisher.updateNextWaypoint()
	...
	if Kingfisher.checkIfLastWaypoint(): # if reached last waypoint
	    thr,rotation = Kingfisher.POShold()
	    sendMotorCommand(pub,thr,rotation)   # headingVal is differential throttle
	else:
	    headingVal = Kingfisher.headingPID() # using PID to reach the target heading
	    sendMotorCommand(pub,cruiseThrottle,headingVal)   # headingVal is differential throttle
